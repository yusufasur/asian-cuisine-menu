...
const buildMenu = (menuArr) => {
  ...
  menuArr.forEach((menuItem) => {
    if (menuItem) {
      ...
      `;
      document.querySelector(".section-center").innerHTML += menuItemDOM;
    }
  });
};
  ...
  filterButtons.forEach((filterButton) => {
    filterButton.addEventListener("click", () => {
      const filteredMenu = menu.map((item) => {
        if (item.category === filterButton.dataset.id) {
          return item;
        } else if (filterButton.dataset.id === "All") {
          return item;
        }
      });
